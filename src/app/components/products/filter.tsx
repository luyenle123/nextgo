...
export default function Filter(props){
  ...
  return (
    <div className={'lg:h-full px-1 sm:px-2 mb-2 lg:mb-0' + blurclass}> 
        ...
        {isActive ? 
            <div className='h-full w-full float-left bg-white py-2 lg:hidden'>
                <FilterListItems isLoading={isLoading} categories={categories} categoryEmptyList={categoryEmptyList} handleCategoryClick={handleCategoryClick} categorySelected={categorySelected}/>
            </div>
        : <></>}
...
export function FilterListItems(props){
    return (
        <>
            ...
            <div className='h-60 pl-2 overflow-x-auto w-full lg:h-auto border sm:border-0 border-gray-100'>
                {!props.isLoading ?                     
                    <>
                        {props.categories.map((p, i) => ( <MapItem key = {i} category = {p} categoryHandleClick = {props.handleCategoryClick} categorySelected={props.categorySelected}/> ))}
                    </>                 
                    :
                    <>
                        {props.categoryEmptyList.map((p, i) => ( <MapEmptyItem key = {i} category = {p}/> ))}
                    </>
                }
            </div>
